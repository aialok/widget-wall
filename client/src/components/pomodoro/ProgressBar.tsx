  ...
  return (
    <div className="relative opacity-80 flex justify-center gap-10 items-center">
      {Array.from({ length: 4 }, (_, i) => (
        <Ball
          key={i}
          status={i < completed ? "COMPLETE" : i === completed ? "ACTIVE" : "INACTIVE"}
        />
      ))}
...
const Ball: FC<BallProps> = ({ status = "INACTIVE" }) => {
  return (
    <div className="z-50 flex items-center justify-center rounded-full">
      ...
      {status === "INACTIVE" && (
        <div className="w-3 h-3 bg-turquoise-tide flex justify-center items-center rounded-full"></div>
      )}
    </div>
  );
};
